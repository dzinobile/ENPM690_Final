...
Usage
-----
  python train_tars_mesh.py                        # fresh run, 4 parallel envs
  python train_tars_mesh.py --timesteps 5000000    # longer run
  python train_tars_mesh.py --n-envs 8             # more parallel envs
  python train_tars_mesh.py --resume checkpoints/tars_ppo_1000000_steps
  python replay_tars_mesh.py --model best_model/best_model  # visualise
"""
...
class TarsEnv(gym.Env):
    """
    MuJoCo environment for TARS (tars_meshed.xml) locomotion.
...
def train(timesteps: int, n_envs: int, resume: str | None):
    ...
    callbacks = [
        ...
        CheckpointCallback(
            ...
            name_prefix="tars_mesh_ppo",
        ),
        EvalCallback(
            eval_env,
            eval_freq=max(25_000 // n_envs, 1),
            n_eval_episodes=5,
            best_model_save_path="./best_model_mesh/",
            verbose=1,
        ),
    ]
    ...
    model.save("tars_mesh_ppo_final")
    print("Training complete — model saved to tars_mesh_ppo_final.zip")
    vec_env.close()
    eval_env.close()
