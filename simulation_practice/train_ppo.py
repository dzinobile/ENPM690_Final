...
-----
  ...
  python train_ppo.py --resume ppo_checkpoint.pkl
  python replay.py --pkl ppo_results.pkl       # visualise
"""
...
# Order matches qpos[7:] / data.ctrl[:]:
...
JOINT_LIMITS_RAD = np.array([
    ...
    [-70,  70],   # br_hip
    [-80,  10],   # br_knee
], dtype=np.float32) * (math.pi / 180.0)

_JT_LO   = JOINT_LIMITS_RAD[:, 0]
_JT_HI   = JOINT_LIMITS_RAD[:, 1]
_JT_MID  = (_JT_LO + _JT_HI) / 2.0
_JT_HALF = (_JT_HI - _JT_LO) / 2.0

# ── PD position controller ────────────────────────────────────────────────────
KP   = 100.0    # position gain
KD   =   5.0    # velocity (damping) gain
GEAR = 120.0    # matches <motor gear="120"> in crawler.xml

# ── Dimensions ────────────────────────────────────────────────────────────────
# Observation: z(1) + quat(4) + joint_pos(8) + lin_vel(3) + ang_vel(3)
#              + joint_vel(8) + prev_action(8) = 35
OBS_DIM = 35
...
def get_obs(data: mujoco.MjData, prev_action: np.ndarray) -> np.ndarray:
    """Build a 35-dim observation vector from MjData + previous action."""
...
def apply_pd_control(data: mujoco.MjData, action: np.ndarray) -> np.ndarray:
    """
    Convert a joint-position action to motor ctrl via a PD controller.
    Returns the raw torques (before gear normalisation) for use in the reward.
    """
    q_target  = action_to_target(action)
    q_current = data.qpos[7:].copy()
    dq        = data.qvel[6:].copy()
    torques   = KP * (q_target - q_current) - KD * dq
    data.ctrl[:] = np.clip(torques / GEAR, -1.0, 1.0)
    return torques
